...
export class Partie {
	...
	run(): void {
        ...
        let partyContinue: boolean = true;
		while (partyContinue) {
	...
	creerHero(): Personnage {
        let nomPerso: string = prompt("Saisir le nom : ");
        let classe: string = prompt("Veuillez choisir une classe : (attaquant/défenseur)");
    ...
    rencontrerDesMechants(): boolean {
        let ennemi: Ennemi = new Ennemi(this.hero.niveau);
        
        let combattre = confirm(`voulez-vous combattre ${ennemi.nom}, il est de niveau ${ennemi.niveau}`);

        if (combattre) {
            
            let nbrCoups: number = 0;
